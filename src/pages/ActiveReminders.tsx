...
import { useLembretes, useCopiarLembrete, useDeleteLembrete, useUpdateLembrete, LembreteRow } from '@/hooks/useLembretes';
...
import { Eye, Copy, Pencil, Ban, Trash2, Search, Plus, Image, Music, FileText, Users, CheckSquare } from 'lucide-react';
...
function toReminder(r: LembreteRow): Reminder {
  return {
    ...
    intervalo_dias: r.intervalo_dias ?? undefined,
    data_envio_especifica: r.data_envio_especifica ?? undefined,
    hora_envio: r.hora_envio,
    categoria: r.categoria?.nome || 'Sem categoria',
    mensagem: r.mensagem,
    status: r.status,
    recorrente: r.recorrente,
    recorrencia_tipo: r.recorrencia_tipo ?? undefined,
    recorrencia_repeticoes: r.recorrencia_repeticoes ?? undefined,
    data_envio_realizado: r.data_envio_realizado ?? undefined,
    data_arquivamento: r.data_arquivamento ?? undefined,
    motivo_erro: r.motivo_erro ?? undefined,
    observacoes: r.observacoes ?? undefined,
    anexo_imagem_url: r.anexo_imagem_url ?? undefined,
    anexo_audio_url: r.anexo_audio_url ?? undefined,
...
export default function ActiveReminders() {
  ...
  const { data: allLembretes = [], isLoading } = useLembretes(['aguardando', 'erro', 'clonado']);
  const { data: categorias = [] } = useCategorias();
  const copiar = useCopiarLembrete();
  const deletar = useDeleteLembrete();
  const update = useUpdateLembrete();
    ...
    return allLembretes
      .filter((r) => {
        if (search) {
          ...
          return r.nome.toLowerCase().includes(q) || r.whatsapp.includes(q);
        }
        return true;
      })
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <h1 className="text-4xl font-bold tracking-tight">Lembretes Ativos <span className="text-muted-foreground text-2xl font-normal">({reminders.length})</span></h1>
          <div className="flex gap-2 flex-wrap">
            ...
            <Link to="/multiplo">
              <Button variant="outline" className="gap-2 font-semibold border-2"><Users className="h-4 w-4" /> Múltiplo Envio</Button>
            </Link>
            <Link to="/novo">
              <Button className="gap-2 font-semibold"><Plus className="h-4 w-4" /> Novo Lembrete</Button>
            </Link>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-card border p-4 space-y-3">
          <div className="relative">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por nome ou telefone..." className="pl-10 h-12 border-2" />
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            <Button variant="outline" onClick={clearFilters} className="border-2">Limpar</Button>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-4 font-semibold hidden lg:table-cell">Data Envio</th>
                  <th className="text-left p-4 font-semibold hidden md:table-cell">Categoria</th>
                  <th className="text-left p-4 font-semibold hidden xl:table-cell">Mensagem</th>
              ...
              <tbody>
                {isLoading ? (
                  Array.from({ length: 3 }).map((_, i) => (
                    <tr key={i}><td colSpan={selectionMode ? 8 : 7} className="p-4"><Skeleton className="h-10" /></td></tr>
                  ))
                ) : reminders.length === 0 ? (
                  <tr><td colSpan={selectionMode ? 8 : 7} className="text-center py-12 text-muted-foreground">Nenhum lembrete encontrado.</td></tr>
                ) : (
                  reminders.map((r) => (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      <td className="p-4"><StatusBadge status={r.status} onStatusChange={(newStatus) => update.mutate({ id: r.id, status: newStatus })} /></td>
                      <td className="p-4">
                        <div className="font-medium">{r.nome}</div>
                        {(r.anexo_imagem_url || r.anexo_audio_url || r.anexo_pdf_url) && (
                          <div className="flex items-center gap-1.5 mt-1">
                            {r.anexo_imagem_url && <span className="text-primary"><Image className="h-3.5 w-3.5" /></span>}
                            {r.anexo_audio_url && <span className="text-primary"><Music className="h-3.5 w-3.5" /></span>}
                            {r.anexo_pdf_url && <span className="text-destructive"><FileText className="h-3.5 w-3.5" /></span>}
                          </div>
                        )}
                      </td>
                      <td className="p-4 hidden md:table-cell text-muted-foreground">{r.whatsapp}</td>
                      <td className="p-4 hidden lg:table-cell text-muted-foreground">
                        {format(parseDateLocal(r.data_contato), 'dd/MM/yyyy')}
                      </td>
                      <td className="p-4 hidden md:table-cell"><CategoryBadge categoria={r.categoria?.nome || 'Sem categoria'} /></td>
                      <td className="p-4 hidden xl:table-cell text-muted-foreground max-w-[200px] truncate">{r.mensagem}</td>
                      <td className="p-4" onClick={(e) => e.stopPropagation()}>
                        <div className="flex items-center justify-end gap-1">
                          {!selectionMode && (
                            <>
                              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setSelectedReminder(toReminder(r))} title="Ver detalhes"><Eye className="h-4 w-4" /></Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8" title="Editar" asChild><Link to={`/novo?id=${r.id}`}><Pencil className="h-4 w-4" /></Link></Button>
                              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => copiar.mutate(r.id)} title="Copiar"><Copy className="h-4 w-4" /></Button>
                              {r.status === 'aguardando' && (
                                <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => update.mutate({ id: r.id, status: 'cancelado' })} title="Cancelar"><Ban className="h-4 w-4" /></Button>
                              )}
                              <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive hover:text-destructive" onClick={() => deletar.mutate(r.id)} title="Deletar"><Trash2 className="h-4 w-4" /></Button>
                            </>
                          )}
                        </div>
                      </td>
