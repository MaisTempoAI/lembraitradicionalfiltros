...
import { useLembretes, useCopiarLembrete, useArquivarLembrete, useDeleteLembrete, useUpdateLembrete, LembreteRow } from '@/hooks/useLembretes';
...
import { Eye, Copy, Archive, Search, Trash2, CheckSquare } from 'lucide-react';
...
export default function FinishedReminders() {
  ...
  const { data: allLembretes = [], isLoading } = useLembretes(['enviado', 'erro', 'cancelado']);
  const { data: categorias = [] } = useCategorias();
  const copiar = useCopiarLembrete();
  const arquivar = useArquivarLembrete();
    ...
    <Layout>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <h1 className="text-4xl font-bold tracking-tight">Lembretes Finalizados <span className="text-muted-foreground text-2xl font-normal">({reminders.length})</span></h1>
        ...
        <div className="rounded-xl bg-card border p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button variant="outline" onClick={() => { setSearch(''); setStatusFilter('all'); setCatFilter('all'); setDateFilter('all'); }} className="border-2">Limpar</Button>
          </div>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-muted/50">
                  ...
                  <th className="text-left p-4 font-semibold hidden lg:table-cell">Enviado em</th>
              ...
              <tbody>
                {isLoading ? (
                  ...
                  <tr><td colSpan={selectionMode ? 7 : 6} className="text-center py-12 text-muted-foreground">Nenhum lembrete finalizado.</td></tr>
                ) : (
                  reminders.map((r) => (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      <td className="p-4">
                        <Select
                          value={r.status}
                          onValueChange={(value) => updateLembrete.mutate({ id: r.id, status: value })}
                        >
                          <SelectTrigger className="w-[130px] h-8 border-0 bg-transparent p-0 shadow-none focus:ring-0 [&>svg]:ml-1">
                      ...
                      <td className="p-4 hidden lg:table-cell text-muted-foreground">
                        {r.data_envio_realizado ? format(new Date(r.data_envio_realizado), 'dd/MM/yyyy HH:mm') : '—'}
                      </td>
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {!selectionMode && (
                            <>
                              ...
                              <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => arquivar.mutate(r.id)}><Archive className="h-4 w-4" /></Button>
                            </>
                          )}
                        </div>
                      </td>
